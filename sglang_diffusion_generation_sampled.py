...
def parse_args():
    parser = argparse.ArgumentParser(description="VBench Video Generation Script with SGLang (Sampled)")
...
def sample_prompts(prompt_list, sample_ratio, sample_seed):
    ...
    Sampling is done using a separate random generator to ensure reproducibility.
    ...
    # Sample without replacement (each prompt appears at most once)
    sampled = rng.sample(prompt_list, num_to_sample)
...
def main():
    ...
    if args.sample_ratio < 1.0:
        ...
        args.save_path += f"_sample{args.sample_ratio:.2f}_seed{effective_sample_seed}"
    ...
    for dimension in args.dimensions:
        ...
        # Sample prompts for this dimension independently
        original_count = len(prompt_list)
        sampled_prompts = sample_prompts(prompt_list, args.sample_ratio, effective_sample_seed)
        sampled_count = len(sampled_prompts)
